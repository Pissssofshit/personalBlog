	...
	<form action="" method="post">
		<ul class="list_A">
			<li><label class="display_name">id:</label><span>{{$detail->id}}</span></li><li><label class="display_name">类型名:</label><span>{{$detail->name}}</span></li><?php $state= isset($dict_boolean[$detail.state])?$dict_boolean[$detail.state]:'';?><li><label class="display_name">是否启用:</label><span>{{$state}}</span></li><li><label class="display_name">具体描述:</label><span>{{$detail->describe}}</span></li><li class="clearfix"></li><li class="ml35"><input class="kbutton" type="button" value="编辑" id="btn_edit"  onclick="javascript:location.href='./{{$detail->id}}/edit'"/><input class="kbutton" type="button" value="删除" id="btn_delete"  onclick="javascript:confirm('确认需要删除此记录！！！')? deleteItem('/autophp/rem_channel_type/{{$detail->id}}', {'_method':'DELETE'}) : null;"/></li>
		</ul>
	</form>
</fieldset>
...
<script type="text/javascript">
	function deleteItem(url, params) {
		...
		$.post(url, params, function(data){
			...
			location.href="/autophp/rem_channel_type";
