		...
		<div id="main" class="easyui-panel" title="当前位置：权限管理 >> 用户管理">
			<div class="easyui-panel" border="false" style="padding:1px">
				<fieldset>
	<legend>编辑</legend>
	<form action="/autophp/power_user/{{$detail->power_user_id}}" method="post">
		<ul class="list_A">
			<input type="hidden" name="_method" value="PUT"/><input type='hidden' name='_token' value='{{$csrf_token}}' /><input type="hidden" maxlength="10" name="power_user_id" id="power_user_id" value="{{isset($detail['power_user_id'])?$detail['power_user_id']:''}}"></input>
