		...
		<div id="main" class="easyui-panel" title="当前位置：cfg >> 留存标记">
			<div class="easyui-panel" border="false" style="padding:1px">
				<fieldset>
	<legend>编辑</legend>
	<form action="/autophp/cfg_subsist_sign/{{$detail->subsist_day}}" method="post">
		<ul class="list_A">
			<input type="hidden" name="_method" value="PUT"/><input type='hidden' name='_token' value='{{$csrf_token}}' /><input type="hidden" maxlength="10" name="subsist_day" id="subsist_day" value="{{isset($detail['subsist_day'])?$detail['subsist_day']:''}}"></input>
<li><label class="display_name">对应标记</label><input type="text" maxlength="100" name="subsist_num" id="subsist_num" value="{{isset($detail['subsist_num'])?$detail['subsist_num']:''}}"></input></li>
